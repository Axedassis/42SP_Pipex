...
/*   Updated: 2025/01/14 11:20:32 by lsilva-x         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	apply_function(char *cmd, char **envp)
{
	...
	if (execve(path, cmd_splited, envp) == -1)
		handler_error("Unable to run the command\n", 126);
}
...
void	first_processing(char **argv, char **envp, int *fd)
{
	...
	if (fd_opened_file == -1)
		handler_error ("Unable to open the file\n", 0);
	close(fd[0]);
...
void	second_processing(char **argv, char **envp, int *fd)
{
	...
	fd_read_file = open(argv[4], O_WRONLY | O_CREAT | O_TRUNC, 0777);
	if (fd_read_file == -1)
		handler_error ("unable to create output file\n", 0);
	close(fd[1]);
